...
function AccountCreationForm(props) {
    const [accountExistsFlag, setAccountExistsFlag] = useState("");
...
const handleLoginSubmit = async (event) => {
    ...
    try {
        ...
        if (response.status === 200) {
            ...
            console.log('Username already exists:', response.data.message);
            setAccountExistsFlag("Username already exists");
        } else {
    ...
    return (
        <div>
            <h2>Account Setup</h2>
             <p style={{ color: 'red' }}>{accountExistsFlag}</p>
